...
미국시장과 한국시장을 각각 별도 페이지로 점검하는 정적 대시보드 프로젝트입니다.
...
### 1.3 한국시장 고정 6종목 모니터링
- 한국 페이지는 대표주 4개와 요청 종목 2개를 합쳐 6종목을 고정으로 봅니다.
- 현재 구성은 `삼성전자`, `SK하이닉스`, `NAVER`, `현대차`, `아이티센글로벌`, `클로봇`입니다.
- 한국 페이지는 미국 페이지보다 더 단순하게 구성했습니다.
- 실적 일정처럼 품질이 들쭉날쭉한 항목은 과감히 빼고, `KOSPI`, `KOSDAQ`, `KOSPI200`, `원달러`, `크로스 신호` 중심으로 봅니다.
...
현재 대시보드는 미국/한국 페이지 모두 비슷한 흐름을 따르지만, 카드 구성은 시장 특성에 맞게 다르게 조정되어 있습니다.
...
한국 페이지는 여기서 `실적발표일`을 빼고 `시장구분(KOSPI/KOSDAQ)`을 보여줍니다.
...
한국 페이지는 복잡한 breadth 계산 대신, `KOSPI / KOSDAQ / KOSPI200 / 원달러 / 코스닥 상대 참여도`를 더 직접적으로 봅니다.
...
## 4.2 한국 시장 점수
한국 페이지는 100점 만점 기준으로 아래 흐름을 봅니다.
...
한국 페이지는 실적 일정 품질 문제를 줄이기 위해, 현재는 실적 캘린더를 화면 핵심 요소에서 제외했습니다.
