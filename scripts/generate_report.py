...
def latest_market_ai_text(previous_ai_output: dict) -> str:
    if not isinstance(previous_ai_output, dict):
        return "이전 AI 분석 없음"
    ai_analysis = previous_ai_output.get("ai_analysis", {})
...
11. 단기투자 팁은 반드시 한 줄의 완전한 문장으로 채운다. 어떤 상황에서도 초단기 기준의 대응 방향을 분명히 제시하고, 지금 매수/추격/대기/비중축소/손절관리 중 무엇이 맞는지 이유를 붙여 짧고 명확하게 쓴다. 만약 보유 중이라면 이렇게 대응하는 것이 좋다는 식으로 짧은 탈출 전략도 함께 조언한다. `대기` 같은 한 단어만 쓰지 않는다.
...
18. 속보 요약에는 시장에 바로 영향을 줄 수 있는 최신 뉴스나 헤드라인만 1~3개까지 짧게 정리한다. 새 속보가 많지 않더라도 기존 속보 흐름을 이어서 최신 내용으로 압축해 쓴다.
...
5. 각 원소는 아래 키만 가진다.
   ...
   - ai_note: 한두 문장 메모. 가능하면 최신 헤드라인 뉴스 한 가지와 현재 추세를 함께 넣고, 만약 보유 중이라면 이렇게 대응하는 것이 좋다는 식으로 짧은 탈출 전략도 함께 조언한다.
...
def write_ai_outputs(output: dict) -> None:
    ...
    ai_output = generate_market_ai_analysis(output, previous_ai_output)
